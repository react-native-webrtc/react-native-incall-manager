...
class InCallManager {
  ...
  startRingtone(ringtone, vibrate_pattern, ios_category, seconds) {
    ...
    seconds =
      typeof seconds === "number" && seconds > 0 ? parseInt(seconds) : -1; // --- android only, default looping
  ...
  pokeScreen(_timeout) {
    if (Platform.OS === "android") {
      let timeout =
        typeof _timeout === "number" && _timeout > 0 ? _timeout : 3000; // --- default 3000 ms
